...
mkdir -p output
touch /tmp/genes.gtf /tmp/genome.fa

# DNA mapping
WC=`DNA-mapping -i PE_input -o output --tempdir /tmp .ci_stuff/organism.yaml --snakemake_options " --dryrun" | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 734 ]; then exit 1 ; fi
WC=`DNA-mapping -i PE_input -o output --tempdir /tmp .ci_stuff/organism.yaml --snakemake_options " --dryrun" --trim --mapq 20 --dedup --properpairs | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 936 ]; then exit 1 ; fi
WC=`DNA-mapping -i SE_input -o output --tempdir /tmp .ci_stuff/organism.yaml --snakemake_options " --dryrun" | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 684 ]; then exit 1 ; fi
WC=`DNA-mapping -i SE_input -o output --tempdir /tmp .ci_stuff/organism.yaml --snakemake_options " --dryrun" --trim --mapq 20 --dedup --properpairs | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 820 ]; then exit 1 ; fi

# ChIP-seq
WC=`ChIP-seq -d BAM_input --tempdir /tmp --snakemake_options " --dryrun" .ci_stuff/organism.yaml .ci_stuff/ChIP.sample_config.yaml | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 268 ]; then exit 1 ; fi
WC=`ChIP-seq -d BAM_input --tempdir /tmp --snakemake_options " --dryrun" --single-end .ci_stuff/organism.yaml .ci_stuff/ChIP.sample_config.yaml | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 268 ]; then exit 1 ; fi
WC=`ChIP-seq -d BAM_input --tempdir /tmp --snakemake_options " --dryrun" --bigWigType log2ratio .ci_stuff/organism.yaml .ci_stuff/ChIP.sample_config.yaml | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 222 ]; then exit 1 ; fi

# ATAC-seq
WC=`ATAC-seq -d BAM_input --tempdir /tmp --snakemake_options " --dryrun" .ci_stuff/organism.yaml | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 336 ]; then exit 1 ; fi
WC=`ATAC-seq -d BAM_input --tempdir /tmp --snakemake_options " --dryrun" --fragmentSize-cutoff 120 --qval 0.1 .ci_stuff/organism.yaml | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 336 ]; then exit 1 ; fi

# RNA-seq
WC=`RNA-seq -i PE_input -o output --tempdir /tmp --snakemake_options " --dryrun" .ci_stuff/organism.yaml | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 889 ]; then exit 1 ; fi
WC=`RNA-seq -i PE_input -o output --tempdir /tmp --snakemake_options " --dryrun" -m "alignment" .ci_stuff/organism.yaml | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 578 ]; then exit 1 ; fi
WC=`RNA-seq -i PE_input -o output --tempdir /tmp --snakemake_options " --dryrun" -m "alignment,deepTools_qc" --trim .ci_stuff/organism.yaml | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 1052 ]; then exit 1 ; fi
WC=`RNA-seq -i SE_input -o output --tempdir /tmp --snakemake_options " --dryrun" .ci_stuff/organism.yaml | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 828 ]; then exit 1 ; fi
WC=`RNA-seq -i SE_input -o output --tempdir /tmp --snakemake_options " --dryrun" -m "alignment" .ci_stuff/organism.yaml | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 528 ]; then exit 1 ; fi
WC=`RNA-seq -i SE_input -o output --tempdir /tmp --snakemake_options " --dryrun" -m "alignment,deepTools_qc" --trim .ci_stuff/organism.yaml | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 925 ]; then exit 1 ; fi

# HiC - fails
WC=`HiC -i PE_input -o output --tempdir /tmp --snakemake_options " --dryrun" .ci_stuff/organism.yaml | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 522 ]; then exit 1 ; fi
WC=`HiC -i PE_input -o output --tempdir /tmp --snakemake_options " --dryrun" --trim .ci_stuff/organism.yaml | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 724 ]; then exit 1 ; fi
WC=`HiC -i PE_input -o output --tempdir /tmp --snakemake_options " --dryrun" --enzyme DpnII .ci_stuff/organism.yaml | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 522 ]; then exit 1 ; fi
WC=`HiC -i PE_input -o output --tempdir /tmp --snakemake_options " --dryrun" --noTAD .ci_stuff/organism.yaml | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 460 ]; then exit 1 ; fi

# scRNA-seq
WC=`scRNAseq -i PE_input -o output --tempdir /tmp --snakemake_options " --dryrun" .ci_stuff/organism.yaml | tee /dev/stderr | wc -l`
if [ ${PIPESTATUS[0]} -ne 0 ] || [ $WC -ne 1080 ]; then exit 1 ; fi
WC=`scRNAseq -i PE_input -o output --tempdir /tmp --snakemake_options " --dryrun" --skipRaceID --split_lib .ci_stuff/organism.yaml | tee /dev/stderr | wc -l`
...
rm -rf SE_input PE_input BAM_input output /tmp/genes.gtf /tmp/genome.fa
