...
if genes_gtf.lower().find("gencode") >=0:
    ...
    rule create_annotation_bed:
        ...
        shell:
            ...
            """ pos=match($0,"gene_id.[^[:space:]]*"); gid=substr($0,RSTART,RLENGTH); """
            """ pos=match($0,"transcript_id.[^[:space:]]*"); tid=substr($0,RSTART,RLENGTH); """
            """ pos=match($0,"transcript_name.[^[:space:]]*"); if (pos!=0) tna=substr($0,RSTART,RLENGTH); else tna="transcript_name "tid; """
            """ pos=match($0,"gene_name.[^[:space:]]*"); if (pos!=0) gna=substr($0,RSTART,RLENGTH); else gna="gene_name "gid; """
