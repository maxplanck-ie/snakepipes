...
DESeq_downstream <- function(DEseqout,
	...
	if (file.exists(geneNamesFile)) {
		...
		geneNames[,1] = NULL
		colnames(geneNames) <- "external_gene_name"
		ddr.df <- merge(ddr.df, geneNames, by = 0, all.x = TRUE)
	} else {
		print("Gene IDs used")
	}
	...
	if (nrow(df.filt) > 0) {
		d <- data.frame(id = df.filt[,1], padj = df.filt$padj)
		...
		htdat <- merge(ddr.df[,c("Row.names","external_gene_name")], heatmap_data, 
					    by.x = 1, by.y = 0, all.y = TRUE, order = FALSE)
