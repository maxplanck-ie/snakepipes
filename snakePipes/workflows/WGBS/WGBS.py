...
def minOne(x):
    ...
    if x < 1:
        raise argparse.ArgumentTypeError(
            "Minimum value of 1 for --maxDist and --minCpGs"
        )
    return x
...
def bounded01(x):
    ...
    if x < 0 or x >= 1:
        raise argparse.ArgumentTypeError(
            "The values for --minMethDiff and --FDR must be between 0 and 1"
        )
    return x
...
def parse_args(
    ...
    optional.add_argument(
        "--minCoverage",
        type=int,
        help="The minimum coverage needed for across all samples for a CpG to be used in DMR calling and PCA. Note that you can change this value without overwriting the DMR output.  Default: '%(default)i'",
    ...
    optional.add_argument(
        "--skipDOC",
        action="store_true",
        help="Skip depth of coverage calculation with deepTools.",
        default=defaults["skipDOC"],
    )
