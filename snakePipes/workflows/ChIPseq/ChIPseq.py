...
def parse_args(
    ...
    parser.add_argument(
        "samples_config",
        nargs="?",
        metavar="SAMPLESCONFIG",
        help="configuration file (eg. 'example.chip_samples.yaml') with sample annotation",
    )
    ...
    optional.add_argument(
        ...
        choices=["MACS2", "Genrich", "SEACR", "histoneHMM"],
    ...
    optional.add_argument(
        "--spikeinExt",
        dest="spikeinExt",
        help="Extension of spikein chromosome names in the hybrid genome. Ignored if useSpikeInForNorm is False (default: '%(default)s') .",
    ...
    optional.add_argument(
        "--FDR",
        dest="fdr",
        help="FDR threshold to apply for filtering DB regions(default: '%(default)s')",
...
def main():
    ...
    if args.predictChIPDict is not None:
        cf.predict_chip_dict(
            args.workingdir, args.predictChIPDict, args.bamExt, args.fromBAM
        )
        sys.exit(0)

    if args.samples_config is not None and os.path.exists(
        os.path.abspath(args.samples_config)
    ):
