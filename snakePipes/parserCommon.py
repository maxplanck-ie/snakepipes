...
def ListGenomes():
    ...
    genomes = [
        ...
        for f in glob.glob(os.path.join(dName, "shared/organisms/*.yaml"))
    ]
    return genomes
...
def mainArguments(defaults, workingDir=False, createIndices=False, preprocessing=False):
    ...
    if not createIndices and not preprocessing:
        ...
        parser.add_argument(
            "genome",
            metavar="GENOME",
            help="Genome acronym of the target organism. Either a yaml file or one of: {}".format(
                ", ".join(genomes)
            ),
        )
    ...
    if workingDir:
        ...
        required.add_argument(
            "-o", "--output-dir", dest="outdir", required=True, help="output directory"
        )
...
def snpArguments(defaults):
    ...
    snpargs.add_argument(
        "--SNPfile", default="", help="File containing SNP locations (default: 'None')"
    )
