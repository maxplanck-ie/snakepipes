...
if bigWigType == "subtract" or bigWigType == "both":
    rule bamCompare_subtract:
        input:
            chip_bam = "split_bam/{chip_sample}.host.bam",
            chip_bai = "split_bam/{chip_sample}.host.bam.bai",
            control_bam = "split_bam/{control_name}.host.bam",
            control_bai = "split_bam/{control_name}.host.bam.bai"
        output:
            "split_deepTools_ChIP/bamCompare/{chip_sample}.filtered.subtract.{control_name}.bw"
        params:
            bwBinSize = bwBinSize,
        ...
        log:
            out = "split_deepTools_ChIP/logs/bamCompare.subtract.{chip_sample}.filtered.subtract.{control_name}.out",
            err = "split_deepTools_ChIP/logs/bamCompare.subtract.{chip_sample}.filtered.subtract.{control_name}.err"
        benchmark:
            "split_deepTools_ChIP/.benchmark/bamCompare.subtract.{chip_sample}.filtered.subtract.{control_name}.benchmark"
        threads: 16
...
if bigWigType == "log2ratio" or bigWigType == "both":
    rule bamCompare_log2:
        input:
            ...
            control_bai = "split_bam/{control_name}.host.bam.bai",
        output:
            "split_deepTools_ChIP/bamCompare/{chip_sample}.filtered.log2ratio.over_{control_name}.bw"
        params:
            bwBinSize = bwBinSize,
            ignoreForNorm = "--ignoreForNormalization {}".format(ignoreForNormalization) if ignoreForNormalization else "",
            read_extension = "--extendReads" if pairedEnd else "--extendReads {}".format(fragmentLength),
            blacklist = "--blackListFileName {}".format(blacklist_bed) if blacklist_bed else ""
        log:
            out = "split_deepTools_ChIP/logs/bamCompare.log2ratio.{chip_sample}.{control_name}.filtered.out",
            err = "split_deepTools_ChIP/logs/bamCompare.log2ratio.{chip_sample}.{control_name}.filtered.err"
        benchmark:
            "split_deepTools_ChIP/.benchmark/bamCompare.log2ratio.{chip_sample}.{control_name}.filtered.benchmark"
        threads: 16
        conda: CONDA_SHARED_ENV
...
rule plotEnrichment:
    input:
        bams = expand("split_bam/{sample}.host.bam", sample = all_samples),
        bais = expand("split_bam/{sample}.host.bam.bai", sample = all_samples)
    output:
