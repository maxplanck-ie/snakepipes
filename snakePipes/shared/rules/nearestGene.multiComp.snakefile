...
rule get_nearest_transcript:
    input:
        bed="CSAW_{}_{}".format(peakCaller, sample_name + "{compGroup}")+"/Filtered.results.{change_dir}.bed"  if pipeline in ['chip-seq','ATAC-seq'] else ""
    output:
        annotated_bed=temp("AnnotatedResults_{}_{}".format(peakCaller, sample_name + "{compGroup}")+"/Filtered.results.{change_dir}_withNearestTranscript.bed")
    params:
        genes_bed=genes_bed,
...
rule get_nearest_gene:
    input:
        bed="AnnotatedResults_{}_{}".format(peakCaller, sample_name + "{compGroup}")+"/Filtered.results.{change_dir}_withNearestTranscript.bed",
        t2g="Annotation/genes.filtered.t2g",
        gene_symbol="Annotation/genes.filtered.symbol"
    output:
        annotated_bed="AnnotatedResults_{}_{}".format(peakCaller, sample_name + "{compGroup}")+"/Filtered.results.{change_dir}_withNearestGene.txt"
    params:
        pipeline=pipeline,
