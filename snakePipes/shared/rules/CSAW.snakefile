...
if allele_info == 'FALSE':
    rule calc_matrix_CSAW_up:
        ...
        params:
            bigwigs = expand("deepTools_ChIP/bamCompare/{chip_sample}.filtered.log2ratio.over_{control_name}.bw",chip_sample=filtered_dict.keys(),control_name=filtered_dict.values()),
            bed_up = "CSAW_{}/Filtered.results.UP.bed".format(sample_name)
        log:
            out = os.path.join(outdir,"CSAW_{}/logs/deeptools_matrix.out".format(sample_name)),
            err = os.path.join(outdir,"CSAW_{}/logs/deeptools_matrix.err".format(sample_name))
        threads: 8
    ...
    rule plot_heatmap_CSAW_up:
        input:
            matrix = "CSAW_{}/CSAW.UP.matrix".format(sample_name)
        output:
            image = "CSAW_{}/CSAW.UP.hatmap.png".format(sample_name),
            sorted_regions = "CSAW_{}/CSAW.UP.sortedRegions.bed".format(sample_name)
        log:
        ...
        conda: CONDA_SHARED_ENV
        shell: "if [ -r {input.matrix} ]; then plotHeatmap --matrixFile {input.matrix} --outFileSortedRegions {output.sorted_regions} --outFileName {output.image};fi >{log.out} 2>{log.err}"
