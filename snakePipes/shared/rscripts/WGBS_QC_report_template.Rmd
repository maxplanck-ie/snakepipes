...
if (length(snakemake@input[["ConversionRate"]] > 0)) {
    ...
    samples = as.character(sapply(snakemake@input[["ConversionRate"]], function(x)gsub(".rrbs_summary_metrics","",basename(x)) ))
    stats<-data.frame("Sample"=samples,"Convertion Rate (%)"=as.numeric(sapply(d$V4,function(X)as.numeric(X[2])*100)))
