...
# Annotated DMRs
################################
# Make annotate with the nearest gene and its distnace
...
d = t(sapply(split(og, queryHits(og)), function(x) {
      ...
      return(as.character(c(gnames, distances)))
}))
gr$NearestGene = as.character(d[,1])   
gr$DistanceToNearestGene = as.character(d[,2])
