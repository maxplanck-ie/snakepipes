...
PCA plot calculated on single CpGs used as input to metilene is stored in **`r outputPCA`**. Typically, the first principal component represents the auto-correlation in methylation values for single CpGs. If the biological effect of the experiment on DNA methylation is sufficiently large, the samples should split by biological condition on the second principal component.

```{r echo=FALSE, warning=FALSE, message=FALSE}
# Select the mean* columns, strip "mean" and make long
d = read.table(PCAinput,header=TRUE,sep="\t",quote=FALSE,as.is=FALSE)
d = subset(d,select=!colnames(d) %in% "chr","pos")
x1<-FactoMineR::PCA(d,graph=FALSE)
if(nrow(x1$eig)>=2){
png(paste0("PCA.png"),bg="white") 
