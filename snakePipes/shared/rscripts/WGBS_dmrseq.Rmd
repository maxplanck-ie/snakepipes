...
message(sprintf("Samples belonging to group 1 (%s) are %s",groups[1],paste(ss$name[g1])))
message(sprintf("Samples belonging to group 2 (%s) are %s",groups[2],paste(ss$name[g2])))
```
...
```{r warning=FALSE, message=FALSE}
if (length(regions) > 0) {
  g = ggplot(as.data.frame(regions,stringsAsFactors=FALSE), aes(x=pval)) + geom_histogram(binwidth=0.005) + labs(x="Unadjusted p-value")
  print(g)
} else {
  message('No DMRs found.')
}
