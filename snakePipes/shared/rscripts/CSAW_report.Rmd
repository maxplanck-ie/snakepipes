...
The absolute log fold change of the window with smallest pvalue belonging to a merged region was reported as best log fold change for that region.
...
Applying FDR threshold of `r params$fdr` and absolute (best) log fold change threshold of `r params$lfc` result in a final list of `r nrow(full_res.filt)` merged regions.   
...
```{r plot_l2r, fig.cap=c("UP","DOWN")}
if(!params$pipeline %in% "chip-seq"){message("ChIP signal enrichment over input is only available for ChIP-seq workflow!")}else{
    knitr::include_graphics(file.path(params$outdir,"CSAW.UP.log2r.heatmap.png"))
    knitr::include_graphics(file.path(params$outdir,"CSAW.DOWN.log2r.heatmap.png"))}
...
```{r plot_cov, fig.cap=c("UP","DOWN")}
knitr::include_graphics(file.path(params$outdir,"CSAW.UP.cov.heatmap.png"))
knitr::include_graphics(file.path(params$outdir,"CSAW.DOWN.cov.heatmap.png"))
```

Tables with differentially bound regions annotated with distance to nearest gene can be found in folder `r paste0("AnnotatedResults_",gsub("\\.[a-z]{3}$","",basename(params$sample_sheet)))`.
