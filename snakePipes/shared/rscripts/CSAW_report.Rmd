...
Applying FDR threshold of `r params$fdr` and absolute (best) log fold change threshold of `r params$lfc` resulted in a final list of `r nrow(full_res.filt)` merged regions.   
...
if(!params$pipeline %in% "chip-seq"){message("ChIP signal enrichment over input is only available for ChIP-seq workflow!")}else{
    knitr::include_graphics(rbind(file.path(params$outdir,"CSAW.UP.log2r.heatmap.png"),file.path(params$outdir,"CSAW.DOWN.log2r.heatmap.png")))
    }
...
knitr::include_graphics(rbind(file.path(params$outdir,"CSAW.UP.cov.heatmap.png"),file.path(params$outdir,"CSAW.DOWN.cov.heatmap.png")))
```
