...
def parseArgs():
    parser = argparse.ArgumentParser(
        description="Counts number of 5' ends in each region of a BED-file."
    )
    parser.add_argument(
        "forward",
        help="Strand-specific bigwig file (forward strand) with 5' signal of _R1.",
    )
    parser.add_argument(
        "reverse",
        help="Strand-specific bigwig file (reverse strand) with 5' signal of _R1.",
    )
    parser.add_argument("bedfile", help="Input bed file.")
    parser.add_argument(
        "output", help="Output file number of 5' read counts for each region."
    )
    return parser
