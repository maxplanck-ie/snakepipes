            ...
            if count > minReads:
                if gene not in clusters:
                    ...
                    for c in clusters[gene]:
                        if (
                            pas > clusters[gene][c][0] - window
                            and pas < clusters[gene][c][1] + window
                        ):
...
def parseArgs():
    parser = argparse.ArgumentParser(
        description="Clusters poly(A) sites that are supported by at least m reads within a given distance w. M can be changed with --minReads, w with --windowSize."
    )
    ...
    parser.add_argument(
        "--windowSize",
        help="Maximal distance between PAS to be clustered (default: 15).",
        default=15,
        type=int,
    )
    parser.add_argument(
        "--minReads",
        help="Number of reads that define a PAS (default: 5).",
        default=5,
        type=int,
    )
    return parser
