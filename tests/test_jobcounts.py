...
def createTestData(fp, samples=9) -> None:
    ...
    (fp / "allelic_bam_input" / "deepTools_qc" / "bamPEFragmentSize").mkdir(
        parents=True
    )
    (
        fp
    ...
    (fp / "allelic_bam_input" / "split_deepTools_qc" / "multiBamSummary").mkdir(
        parents=True
    )
    (
        fp
        ...
        / "split_deepTools_qc"
        / "multiBamSummary"
        / "spikein.scaling_factors.txt"
    ).touch()
    (fp / "allelic_bam_input" / "split_deepTools_qc" / "bamPEFragmentSize").mkdir(
        parents=True
    )
    (
        fp
