...
dat=data.frame(file=Sys.glob(paste("A732_mapping.gencode.genome/Counts/","/*.coutt.csv",sep="")),stringsAsFactors=F)
...
sample_names <- read.table("A732_cell_names.tsv",sep = "\t",stringsAsFactors = F)
...
#vars=setdiff(names(dat),"cell_idx")
#dat %>% group_by_(.dots=vars) %>% select(sample,plate,library,cell_idx,plate) %>% summarise(min=min(cell_idx),max=max(cell_idx))
...
for (i in 1:length(dat$file)) {
	...
	for (j in 1:nrow(subset)) {	
		...
		## get bounds, +1 dure to GENID column
		min_idx = subset[j,"start_idx"] + 1
		max_idx = subset[j,"end_idx"] + 1
		
		print(paste("subset",prefix_subset,min_idx,max_idx));
		
		colnames(tmp)[min_idx:max_idx] <- sapply(colnames(tmp)[min_idx:max_idx], function(x,t) paste0(t,sub("X","_",x)),t=prefix_subset,simplify="array")
		
		alldata<-merge(alldata,tmp[,c(1,min_idx:max_idx)],by="GENEID",all=T)
	}
}
